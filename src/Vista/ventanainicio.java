...
public class ventanainicio extends JFrame {
        ...
        ActionListener action_inicioseccion = new ActionListener() {
            ...
            public void actionPerformed(ActionEvent ae) {
                ...
                if (iniciarseccion.getText() == source.getText()) {
                    if (usuario_caja.getText().equals("") || contrasena_validar.equals("")
                            || (principal.tipo_usuario == true && codigoadmin_caja.getText().equals(""))) {
                        // Si es que no llena los campos para iniciar seccion se le arrojara un error
                        error_campo.setVisible(true);
                    } else {
                        error_campo.setVisible(false);
                        // Si el usuario(ADMINISTRADOR) llena todos los campos para iniciar seccion
                        // comprobamos si es que el usuario es correcto
                        if (principal.tipo_usuario) {
                            if (validarusuario(usuario_caja.getText(), contrasena_validar,
                                    codigoadmin_caja.getText())) {
                                // Si es que el usuario ingresa correctamente todos los campos, se le abrira la
                                // ventana para que pueda ver el contenido de la empresa
                                principal.v_inicio.setVisible(false);
                                principal.v_principal.init_ventanaprincipal();
                                principal.v_principal.iniciarcomponentes();
                                principal.v_principal.setVisible(true);
                            } else {
                                // si es que el usuario ingresa incorrectamente(porque el usuario no existe),
                                // entonces se le arrojara un error
                                error_campo.setVisible(true);
                            }
                            // Si el usuario(USUARIO) llena todos los campos para iniciar seccion
                            // comprobamos si es que el usuario es correcto
                        } else {
    ...
    public boolean validarusuario(String usuario, String contrasena, String codigoadmin) {
        ...
        int buscarindice = -1;
        // La funcion retornara false si el usuario no es valido , de caso contrario
        // retornara un true si es que es valido
        boolean usuariovalido = false;
        // Buscamos en el arreglo de nombres de usuario, si es que encontramos el nombre
        // de usuario guardamos el indice
        for (int i = 0; i < principal.base_datos.getTamNU(); i++) {
            if (principal.base_datos.getNombres_usuario().get(i).equals(usuario)) {
                buscarindice = i;
            }
        }
        // Si es que el indice fue encontrado entonces validamos que sean correcto los
        // otros datos(contraseña o codigo admin si es que este es ADMINISTRADOR)
        if (buscarindice != -1) {
            // Si el usuario es ADMINISTRADOR
            if (principal.tipo_usuario == true) {
                // Si es que la contraseña y el codigo administrador es la correcta entonces el
                // usuario es valido, de caso contrario no es valido
                if (principal.base_datos.getContrasena_usuario().get(buscarindice).equals(contrasena)
                        && principal.base_datos.getCodigo_admin().get(buscarindice).equals("null")) {
                    usuariovalido = false;
                } else if (principal.base_datos.getContrasena_usuario().get(buscarindice).equals(contrasena)
                        && principal.base_datos.getCodigo_admin().get(buscarindice).equals(codigoadmin)) {
                    usuariovalido = true;
                }
                // Si el usuario es USUARIO
            } else {
                // Si es que la contraseña es la correcta entonces el usuario es valido, de caso
                // contrario no es valido
                if (principal.base_datos.getContrasena_usuario().get(buscarindice).equals(contrasena)
                        && principal.base_datos.getCodigo_admin().get(buscarindice).equals("null")) {
                    usuariovalido = true;
                } else {
